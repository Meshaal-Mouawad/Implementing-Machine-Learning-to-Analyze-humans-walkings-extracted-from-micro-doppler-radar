...
for n = 1:Nfeatures
   ...
   % Train
   svm_model = svmtrain(XXX, sparse(Xtrain_feature), libsvm_train_str);

   %
   % Get results on training and testing set, and plot the discriminant function
   %
   train_pred = svmpredict(XXX, sparse(Xtrain_feature), svm_model, '-q');
   fprintf("\nTraining confusion matrix and stats (only feature %d):\n", n);
   [~, train_oa, ~, ~, ~, ~] = confusion_matrix(train_pred, Ytrain, 1, 1);
   
   test_pred = svmpredict(XXX, sparse(Xtest_feature), svm_model, '-q');
...
% Train
svm_model = svmtrain(XXX, sparse(Xtrain), libsvm_train_str);

%
% Get results on training and testing set, and plot the discriminant function
%
train_pred = svmpredict(XXX, sparse(Xtrain), svm_model, '-q');
...
test_pred = svmpredict(XXX, sparse(Xtest), svm_model, '-q');
