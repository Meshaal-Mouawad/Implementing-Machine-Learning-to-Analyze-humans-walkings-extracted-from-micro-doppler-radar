...
addpath('C:\Users\Meshaal Mouawad\Desktop\MSU-Getac2\Fall 2020\DSP\MP2\MP2_Student\libsvm-3.23')
...
for n = 1:Nfeatures
   ...
   %
   train_pred = svmpredict(Xtest_feature, sparse(Xtrain_feature), svm_model, '-q');
...
% Train
svm_model = svmtrain(Xtrain_feature, sparse(Xtrain), libsvm_train_str);

%
% Get results on training and testing set, and plot the discriminant function
%
train_pred = svmpredict(Xtest_feature, sparse(Xtrain), svm_model, '-q');
...
test_pred = svmpredict(Xtest_feature, sparse(Xtest), svm_model, '-q');
